...
var getBlockOfFileCharacteristic,
...
const BleFileTransferExample = () => {
  ...
  const transferFile = async () => {
    ...
    try {
      let fileLengthArray = new TextEncoder().encode(
        fileContents.byteLength.toString()
      );
  ...
  const getTransferedFiled = async () => {
    try {
      ...
      await getBlockOfFileCharacteristic.writeValue(
      ...
      async function callableFileTransfer() {
        ...
        if (decodeFileLength >= decodeUnitMaintain) {
          ...
          console.log(value);
          await getBlockOfFileCharacteristic.writeValue(
            new TextEncoder().encode((decodeUnitMaintain + 1).toString())
          );
          ...
          setStatus(
            "File block written - " +
              (decodeFileLength - decodeUnitMaintain) * 128 +
              " bytes remaining"
          );
      ...
      <div>
        ...
        <label htmlFor="fileInput" className="custom-file-upload">
          ...
          <input
            ...
            id="fileInput"
            onChange={(e) =>
              e.target.files.length ? setFileName(e.target.files[0].name) : null
            }
            style={{ display: "none" }}
          />
        </label>
